  ...
  setdataLabelSets(): void {
    ...
    for (let i = 0; i < overallMetricsLayout.length; i++) {
      ...
      if (metricsFormat === 'currency') {
        dataLabelSet.push(
          this.currencyPipe.transform(overallMetricsData[metricKey]),
          'USD'
        );
      } else if (metricsFormat === 'percent') {
        dataLabelSet.push(
          this.percentPipe.transform(overallMetricsData[metricKey], '1.2-2')
        );
      } else {
