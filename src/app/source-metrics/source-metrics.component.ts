...
export class SourceMetricsComponent {
  ...
  getSourceMetricsLabel(): void {
    this.metricsService.getSourceMetricsLabel().subscribe(label => this.label = label);
  }

  getSourceMetricsLayout(): void {
    this.metricsService.getSourceMetricsLayout().subscribe(layout => this.layout = layout);
  }
